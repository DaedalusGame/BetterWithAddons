...
import betterwithaddons.lib.AtlasSprite;
...
public class RTEventHandler
{
	...
	public void onEntityDrop(LivingDropsEvent event) {
		...
		if (living instanceof EntityShulker){
			if (rand.nextFloat() < 1.0f) {
				//event.getEntityLiving().dropItem(ModItems.material.getMaterial("ender_cream"), 1+rand.nextInt(2));
			}
		}
	}
	...
	public void breakBlock(PlayerEvent.BreakSpeed breakEvent)
	{
		...
		System.out.println("total hardness:"+hardness);
	...
	public void textureStitch(TextureStitchEvent.Pre event)
	{
		try
		{
			ASMDataTable asmData = BetterWithAddons.instance.getASMData();
		...
		{
			BetterWithAddons.instance.logger.log(Level.ERROR, "Error stitching extra textures");
			e.printStackTrace();
		}
	}
