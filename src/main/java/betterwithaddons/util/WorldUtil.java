...
{
	public static int getAdjacentBlocks(World world, BlockPos blockpos, Predicate<IBlockState> blockpredicate)
	{
		BlockPos[] positions = new BlockPos[] {blockpos.up(),blockpos.down(),blockpos.north(),blockpos.east(),blockpos.south(),blockpos.west()};
		int n = 0;

		for(BlockPos nextpos : positions)
		{
			n += blockpredicate.apply(world.getBlockState(nextpos)) ? 1 : 0;
		}
	...
	public static int getSkylight(World world, BlockPos pos)
	{
		...
		float celesang = world.getCelestialAngleRadians(1.0F);

		if(skylight > 0) {
			float lvt_6_1_ = celesang < 3.1415927F?0.0F:6.2831855F;
			celesang += (lvt_6_1_ - celesang) * 0.2F;
			skylight = Math.round((float)skylight * MathHelper.cos(celesang));
		}

		return MathHelper.clamp_int(skylight, 0, 15);
	}
}
